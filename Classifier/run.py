...
if __name__ == '__main__':
     ...
     ret = True   
     model = tf.keras.models.load_model('/home/donkeycar1/mycar/models/0.35_4000imgs_cut.h5', compile=False)
     ...
     while True:
         ...
         img = np.mean(img,axis = 2)[80:224,:]
         
         img[np.where(img>threshold)] = 255
         img[np.where(img<=threshold)] = 0
         cv2.imwrite("/home/donkeycar1/images/"+ str(count) + '.jpg',img)
          
         
         out = model(img[None,:])


         throttle = 0.34 #(out[0][0][0].numpy())
         ...
         if throttle>0:
             ...
             throttle_pulse = map_range(throttle, 0, 1,THROTTLE_STOPPED_PWM, THROTTLE_FORWARD_PWM)
         else:

             throttle_pulse = map_range(throttle, -1, 0,
                                            THROTTLE_REVERSE_PWM, THROTTLE_STOPPED_PWM)
                                            

         steering_pulse = map_range(steering,
                                        -1, 1,
                                        STEERING_LEFT_PWM,
                                        STEERING_RIGHT_PWM)
